...
import { View, Text, TextInput, StyleSheet, Pressable, ScrollView, Alert, ActivityIndicator, Modal, FlatList } from 'react-native';
...
export default function EditFlowerScreen() {
    ...
    const loadData = async () => {
        try {
            const [flower, suppliersData, cats] = await Promise.all([
                InventoryService.getFlowerById(id),
            ...
            if (flower) {
                ...
                setFreshnessDays(flower.freshness_days.toString());
                setSelectedSupplierId(flower.supplier_id);
            }
            setSuppliers(suppliersData);
            setCategories(cats);
        } catch (e) {
            console.error(e);
            Alert.alert('Error', 'Failed to load flower data');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleUpdate = async () => {
        ...
        if (!name || !price || !quantity || !freshnessDays) {
            Alert.alert('Error', 'Please fill all required fields');
            return;
        }

        try {
            await InventoryService.updateFlower(id, {
                name,
                color,
            ...
            Alert.alert('Error', 'Failed to update flower');
            console.error(error);
        }
    };
        ...
        Alert.alert(
            ...
            [
                { text: 'Cancel', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        try {
                            await InventoryService.deleteFlower(id);
                            router.back();
                        } catch (e) {
    ...
    return (
        <ScrollView style={styles.container}>
            <View style={styles.headerRow}>
                <Text style={styles.headerTitle}>Edit Flower</Text>
            ...
            <View style={styles.formGroup}>
                ...
                <TextInput style={styles.input} value={freshnessDays} onChangeText={setFreshnessDays} keyboardType="numeric" />
            </View>
            ...
            <Pressable style={styles.button} onPress={handleUpdate}>
                <Text style={styles.buttonText}>Update Flower</Text>
            </Pressable>
